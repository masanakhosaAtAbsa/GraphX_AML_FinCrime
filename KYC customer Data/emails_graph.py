...
class TopTen__email_address_GraphGenerator:
    def __init__(self, cleaned_data):
        self.cleaned_data = cleaned_data
        self.G = nx.Graph()  # Create a directed graph
        self.components = None

    def email_address_generate_top_ten_graph(self):
        for _, row in self.cleaned_data.iterrows():
            # Add node for each ID

            self.G.add_node(row['id_number'])

            email_neighbors = self.cleaned_data[self.cleaned_data['email_address'] == row['email_address']]['id_number'].tolist()
            for neighbor in email_neighbors:
    ...
    def create_email_address_pvis_network(self,top_ten_graph):
        ...
        cluster_emails_number = {}
        for i, component in enumerate(self.components):
            cluster_emails_in_component = set(self.cleaned_data.loc[self.cleaned_data['id_number'].isin(component), 'email_address'])
            cluster_emails_number[i + 1] = str(cluster_emails_in_component).strip('{}')
        # Add a single label for each connected component
        # Add a single label for each connected component based on addresses
        for i, component in enumerate(self.components):
            cluster_emails = cluster_emails_number[i + 1]
            nt.add_node(cluster_emails, label=cluster_emails, color='gray', size=10)  # Optional: Customize appearance of cluster nodes
            for node in component:
                # Check if the source and target nodes are different before adding the edge
                if node != cluster_emails:
                    nt.add_edge(cluster_emails, node)

        # Save the graph visualization as an HTML file
        nt.save_graph('one_label_per_connected_component_email_address.html')